...
# --- 1. LIVE CLOCK & NOTEPAD ---
clock_widget = """
<div id="clock" style="
    position: fixed; top: 15px; right: 20px; background-color: #1E1E1E; color: #00FF00;
    padding: 8px 15px; border-radius: 8px; font-family: 'Courier New', Courier, monospace;
    font-size: 16px; font-weight: bold; z-index: 99999; box-shadow: 0px 4px 6px rgba(0,0,0,0.5);
...
notes = st.sidebar.text_area("Yahan notes likhein:", value=st.session_state['notepad_text'], height=250)
st.session_state['notepad_text'] = notes
# --- 2. HINA STRATEGY CORE FUNCTIONS ---
SECTORS = {
    "^NSEBANK": "NIFTY BANK", "^NSENIFTY": "NIFTY 50", "^CNXFMCG": "NIFTY FMCG",
    "^CNXIT": "NIFTY IT", "^CNXMETAL": "NIFTY METAL", "^CNXPHARMA": "NIFTY PHARMA", "^CNXPSUBANK": "NIFTY PSU"
}
...
def analyze_stock(ticker):
    try:
        ...
        if np.isnan(close) or np.isnan(e20) or np.isnan(e75): return None 
        ...
        elif close < e20: signal = "❌ SELL"
        return {"STOCK": f"{clean_t} ({index_name})", "PRICE": round(close, 2), "EMA-9": round(e9, 2), "EMA-20": round(e20, 2), "EMA-75": round(e75, 2), "SUPPORT": round(supp, 2), "TARGET": round(res, 2), "VOL(x)": round(vol_x, 1), "SIGNAL": signal}
    except Exception as e: return None
        # --- 3. NEW EMA CROSSOVER STRATEGY FUNCTIONS ---
def analyze_stock_ema(symbol, timeframe):
    try:
        ...
        if crossover_happened and is_currently_bullish:
            ...
            return {"Sector": sector, "Current Price": round(current_candle['Close'], 2), "50 EMA": round(current_candle['EMA_50'], 2), "200 EMA": round(current_candle['EMA_200'], 2), "RSI": round(current_candle['RSI'], 2), "RSI MA": round(current_candle['RSI_MA'], 2), "VWAP": round(current_candle[vwap_col], 2), "Volume": vol_text, "Action": "BUY"}
    except Exception as e: return None
    return None
...
def highlight_table_ema(val):
    if val == "BUY": return 'background-color: rgba(0, 255, 0, 0.3); color: white; font-weight: bold'
    elif isinstance(val, (int, float)): return 'color: #D3D3D3;'
    return ''
...
with tab1:
    ...
    for i, (ticker, name) in enumerate(SECTORS.items()):
        ...
        except: cols[i].metric(label=name, value="Error", delta=None)
    ...
    if auto_refresh:
        ...
        st.info("⏰ Auto-Scan Mode ON: Tool khud har 1 ghante mein scan karke Telegram alert bhejega.")
        start_scan = True  
    else:
    ...
    if start_scan:
        try:
            with open(WATCHLIST_FILE, "r", encoding="utf-8") as f: stocks = f.read().splitlines()
            ...
            else: st.warning("⚠️ Koi valid data nahi mila. Watchlist check karein.")
        except FileNotFoundError: st.error(f"❌ '{WATCHLIST_FILE}' file nahi mili! Dhyan rahe ye GitHub repo me ho.")
...
with tab2:
    ...
    if start_scan_ema:
        ...
        if not watchlist_ema: st.warning("Watchlist empty hai ya read nahi ho payi. ('pinshutrade.txt' file check karein)")
        else:
            ...
            if signals_ema:
                ...
                st.download_button(
                    label="📥 Download Results as CSV", data=csv_ema,
                    file_name=f'EMA_Swing_Signals_{datetime.date.today()}.csv',
                    mime='text/csv', key='dl_ema_btn'
                )
            else: st.warning("Koi aaisa stock nahi mila jisme last 5 din me 50 & 200 EMA crossover hua ho.")
